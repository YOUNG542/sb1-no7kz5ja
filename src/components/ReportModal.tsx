...
export const ReportModal: React.FC<ReportModalProps> = ({ targetUserId, onClose }) => {
  ...
  const handleSubmit = async () => {
    if (!text && !image) return;  // 텍스트와 이미지를 모두 비워두면 아무것도 제출하지 않음
    setLoading(true);
    ...
    if (image) {
      ...
      await uploadBytes(storageRef, image); // 이미지 업로드
      const downloadURL = await getDownloadURL(storageRef); // 이미지 URL 가져오기
